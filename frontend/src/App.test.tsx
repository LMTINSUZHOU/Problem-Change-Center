...
function inspectResult(
  overrides: Partial<InspectResult> & Pick<InspectResult, "job_id" | "filename">
): InspectResult {
  const result: InspectResult = {
    ...
    format_candidates: [],
    package_scope: "single",
    package_layout: "directory",
    problem_count: 1,
    problems: [{ id: overrides.filename.replace(/\.zip$/i, ""), path: "." }],
    problems_truncated: false,
    supported_targets: [],
    ...overrides
  };
  if (!overrides.supported_targets) {
    result.supported_targets = (
      ["hydro", "icpc", "hoj", "fps", "qduoj", "uoj", "dmoj"] as const
    ).filter((format) => format !== result.detected_format);
  }
  return result;
}
...
describe("App archive selection", () => {
  ...
  it("invalidates the inspected job when a different archive is selected", async () => {
    ...
    mockedInspectZip
      .mockResolvedValueOnce(inspectResult({ job_id: "a".repeat(32), filename: "first.zip" }))
      .mockResolvedValueOnce(inspectResult({ job_id: "b".repeat(32), filename: "second.zip" }));
  ...
  it("opens and closes the format capability matrix as a dialog", async () => {
    const user = userEvent.setup();
    render(<App />);

    const trigger = screen.getByRole("button", { name: "查看格式能力矩阵" });
  ...
  it("supports selecting problems from a detected multi-problem package", async () => {
    ...
    mockedInspectZip.mockResolvedValue(inspectResult({
      job_id: jobId,
      filename: "hydro-contest.zip",
      detected_format: "hydro",
      package_scope: "multi",
      package_layout: "contest",
      problem_count: 2,
      problems: [
        { id: "A", path: "problems/A" },
        { id: "B", path: "problems/B" }
      ],
      supported_targets: ["icpc", "hoj", "fps", "qduoj", "uoj", "dmoj"]
    }));
    mockedStartJob.mockResolvedValue({
      id: jobId,
      status: "queued",
    ...
    await user.upload(
      fileInput!,
      new File(["zip"], "hydro-contest.zip", { type: "application/zip" })
    );
    await user.click(screen.getByRole("button", { name: "上传并检查" }));

    expect(await screen.findByText("多题包 · 2 题 · 比赛结构")).not.toBeNull();
    const targetSelect = screen.getByRole("combobox", { name: "输出格式" }) as HTMLSelectElement;
    expect(Array.from(targetSelect.options, (option) => option.value)).not.toContain("hydro");
    expect(targetSelect.value).toBe("icpc");

    await user.selectOptions(
      screen.getByRole("combobox", { name: "转换范围" }),
      "selected"
    );
    const startButton = screen.getByRole("button", { name: "启动容器转换" });
    expect(await screen.findByText("指定题目模式下至少选择一道题目。")).not.toBeNull();
    expect((startButton as HTMLButtonElement).disabled).toBe(true);

    await user.click(screen.getByRole("checkbox", { name: /A/ }));
    expect(screen.queryByText("指定题目模式下至少选择一道题目。")).toBeNull();
    await user.click(startButton);
